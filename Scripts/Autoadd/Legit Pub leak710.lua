-- Legit.Pub source leak by desktop2677
-- legitK hahah stop selling leak shit 1:1
...
warn("[Legit.Pub] Full-Hitbox Bypass Loaded Success")
...
elseif Build == "PAID" then Color = '#FF0000' Ver = "1.0"
elseif Build == "Personal" then Color = '#0000FF' Ver = "1.1"
end

local Window = Library:CreateWindow({ Size = UDim2.fromOffset(550, 610),
Title = "Legit.<font color=\"#d17bff\">Pub</font> | ".."<font color=\""..Color.."\">"..Build.."</font>".."                               [v"..Ver.."]",
Center = true,
AutoShow = true,
})
...
HBExpanderTab:AddSlider('HBX', {Text='X Size',Default=1,Min=0,Max=4,Rounding=2,Compact=false,Suffix="Size"}):OnChanged(function(Value)
  SizeX = Value
end)

HBExpanderTab:AddSlider('HBY', {Text='Y Size',Default=1,Min=0,Max=6,Rounding=2,Compact=false,Suffix="SIze"}):OnChanged(function(Value)
  SizeY = Value
end)

HBExpanderTab:AddSlider('HBZ', {Text='Z Size',Default=1,Min=0,Max=4,Rounding=2,Compact=false,Suffix="Size"}):OnChanged(function(Value)
  SizeZ = Value
end)
...
GunModsTab:AddLabel('Color'):AddColorPicker('TracerColorH', { Default = Color3.fromRGB(95, 0, 255), Title = 'Tracer Color', })
...
writefile("AstralPro/Skins/"..SkinChoice..".skin",game:GetService("HttpService"):JSONEncode({Id=SkinRBxId,Name=SkinChoice}))
end)
...
LightingTab:AddDropdown('MaterialD', {Values = { 'Default', 'ForceField'},Default = 1,Multi = false,Text = 'Naterial:',Tooltip = 'Arms Material'}):OnChanged(function()
...
MiscTab:AddToggle('', {Text = "Jump Crouch",Default = false,}):AddKeyPicker('JumpCrouchKey', {Default='B',SyncToggleState=true,Mode='Toggle',Text='Jump Crouch',NoUI=false})
local stoprun = false
task.spawn(function()
while true do
...
MiscTab:AddToggle('LootAll',{Text='Loot All',Default=false}):AddKeyPicker('LootAllKey', {Default='P',SyncToggleState=true,Mode='Toggle',Text='Loot All',NoUI=false})
...
local XRAY22 = false
MiscTab:AddToggle('XRAY', {Text = 'XRAY', Default = false}):AddKeyPicker('XRAYKey', {Default='Z',SyncToggleState=true,Mode='Toggle',Text='XRAY',NoUI=false}):OnChanged(function()
...
MenuGroup:AddLabel(' Made By: Legit.Pub', true)
...
if pcall(setclipboard,"go kys im not gonna advertise ur skiddy server") then
...
Library:SetWatermark('Legit.Pub - V1.0') Library:SetWatermarkVisibility(Toggles.WatermarkToggle.Value)
end)
...
SaveManager:SetFolder('LegitPub/folder')
SaveManager:BuildConfigSection(Tabs.UISettings)
SaveManager:IgnoreThemeSettings()

--// Theme manager
ThemeManager:SetLibrary(Library)
ThemeManager:SetFolder('LegitPub/themes')
