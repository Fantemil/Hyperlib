...
spawn(function()
    ...
    msg.Text = "MheeHax is a bozo LLL nigger nigger nigger!"
    task.wait(4)
    msg.Text = "Btw i just fixed the script no additional features were added, MheeHax is a bozo LLL nigger nigger nigger!"
    task.wait(5)
    msg.Text = "Also ill be adding nigger to make the script less reviewable nigger nigger nigger"
    task.wait(5)
    msg:Destroy()
end)    
...
Toggles:AddToggle({
    ...
    Callback = function(kaura)
        spawn(function()
            if kaura then
                _G.killaura = true
                notify("Furry Infection Fucker","KillAura On nigger")
            else
                _G.killaura = false
                notify("Furry Infection Fucker","KillAura Off nigger")
            end
...
Toggles:AddToggle({
    ...
    Callback = function(autogetbatt)
        ...
        if autogetbatt then
            _G.getbatcrack = true
            notify("Furry Infection Fucker","AutoGetBat On nigger")
        else
            _G.getbatcrack = false
            notify("Furry Infection Fucker","AutoGetBat Off nigger")
        end
...
if game.Players.LocalPlayer.Character:FindFirstChild("Bat") or game.Players.LocalPlayer.Backpack:FindFirstChild("Bat") or game.Players.LocalPlayer.Backpack:FindFirstChild("Metal  Bat") or game.Players.LocalPlayer.Character:FindFirstChild("Metal  Bat") then
    ...
    notify("Furry Infection Fucker","Grabbed Bat nigger")
end
end) 
end
end)
...
Toggles:AddToggle({
 ...
 Callback = function(lkillall)
     spawn(function()
         if lkillall then
             _G.killall = true
             notify("Furry Infection Fucker","Loop Kill All On nigger")
         else
             _G.killall = false
             notify("Furry Infection Fucker","Loop Kill All Off nigger")
         end 
 ...
 Callback = function(antigrabbed)
     spawn(function()
         if antigrabbed then
             ...
             notify("Furry Infection Fucker","Auto Escape Grab On nigger")
        else 
            _G.autoescapegrab = false
            notify("Furry Infection Fucker","Auto Escape Grab Off nigger")
         end
 ...
 Callback = function(lesscd)
     spawn(function()
         if lesscd then
             ...
             notify("Furry Infection Fucker","Less Cooldown On nigger")
         else
             _G.lesscooldown = false
             notify("Furry Infection Fucker","Less Cooldown Off nigger")
         end 
        end)
    end
})
