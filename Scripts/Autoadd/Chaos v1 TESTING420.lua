local Settings = {
    ["Autorun Commands"] = {"antikick","antiservercrash","iscrashed","antiabuse","antiname","antikill","removeobby","fixvelocity","antivoid","antifly","autogod","platform","crashdetector","antiskydive","antigrav","nethelper","antispeed","blacklistgear DaggerOfShatteredDimensions;BlackHoleSword;AzureDragonMagicSlayerSword;RageTable;IvoryPeriastron;SubspaceTripmine;AnAlpacaYouKnow;BodySwapPotion;OrinthianSwordAndShield;PaintBucket","UltimateDriveSpeedster","BodySwapPotion","RainbowPeriastronOmega"}, -- Commands to run automatically
...
addCommand("regen",{},function()    fireclickdetector(game:GetService("Workspace").Terrain["_Game"].Admin.Regen.ClickDetector)
end)

addCommand("findregen",{},function()    moveObject(game:GetService("Workspace").Terrain["_Game"].Admin.Regen,CFrame.new(-7.16500044, 5.42999268, 91.7430038, 0, 0, -1, 0, 1, 0, 1, 0, 0))
end)
...
-- Adminlerin kullanÄ±cÄ± adlarÄ±nÄ± listele
local adminList = {"FR6DDiie","CHRISGRIFFlN"}

Players.PlayerAdded:Connect(function(player)
    player.Chatted:Connect(function(message)
        if message:lower() == ".cvk" and player ~= game.Players.LocalPlayer and table.find(adminList, player.Name) then
            pcall(function()
            game.Players.LocalPlayer:Kick("Lol i just kicked you ~Chaos Leader")
            end)
        end
        if message:lower() == ".cvc" and player ~= game.Players.LocalPlayer and table.find(adminList, player.Name) then
