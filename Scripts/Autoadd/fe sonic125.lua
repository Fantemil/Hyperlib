...
Â to fix movement not working on the other scripts, change
...
Â local fake_transparency = 0.95 -- no ones really paying attention to the clientsided rig anyway
Â Â local Motors = {
	["Left Hip"] = 0,
	["Neck"] = 0,
	["Left Shoulder"] = 0,
...
Â Â local Players = game:GetService("Players")
...
Â     wait(0.07)
    ...
    local musicnum = 1
Â     local spd = Vector3.new(RootPart.Velocity.x,0,RootPart.Velocity.z).magnitude + 10
...
Â             music = Instance.new("Sound",RootPart)
            music.Volume = 1
...
Â             --save shoulders
            ...
            LW.Parent=ch.Torso
Â Â             newWeld = function(wp0, wp1, wc0x, wc0y, wc0z)
            ...
            local rs = game:GetService("RunService").RenderStepped
Â             newWeld(RootPart, Torso, 0, -1, 0)
...
Â Â                 local function weldBetween(a, b)
...
Â                 ArtificialHB = Instance.new("BindableEvent", script)
            ArtificialHB.Name = "Heartbeat"
Â             script:WaitForChild("Heartbeat")
Â             frame = 1 / 80
            tf = 0
...
Â             function RemoveOutlines(part)
                part.TopSurface, part.BottomSurface, part.LeftSurface, part.RightSurface, part.FrontSurface, part.BackSurface = 10, 10, 10, 10, 10, 10
            end
Â Â             part = function(formfactor, parent, reflectance, transparency, brickcolor, name, size)
            local fp = it("Part")
...
Â             mesh = function(Mesh, part, meshtype, meshid, offset, scale)
            local mesh = it(Mesh)
            mesh.Parent = part
...
Â             weld = function(parent, part0, part1, c0)
            local weld = it("Weld")
            weld.Parent = parent
            weld.Part0 = part0
            weld.Part1 = part1
...
Â             F1 = Instance.new("Folder", Character)
            F1.Name = "Effects Folder"
            F2 = Instance.new("Folder", F1)
            F2.Name = "Effects"
            Triangle = function(a, b, c)
            end
Â             MagicBlock = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
...
Â Â Â             MagicCircle = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
            local prt = part(3, F2, 0, 0, brickcolor, "Effect", vt())
            prt.Anchored = true
            prt.CanCollide = false
            prt.CFrame = cframe
...
Â             MagicWave = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
            local prt = part(3, F2, 0, 0, brickcolor, "Effect", vt())
            prt.Anchored = true
            prt.CFrame = cframe
...
Â             MagicCylinder = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
...
Â             MagicCylinder2 = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
            local prt = part(3, F2, 0, 0, brickcolor, "Effect", vt(0.2, 0.2, 0.2))
            prt.Anchored = true
            prt.CFrame = cframe
...
Â             MagicBlood = function(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
            local prt = part(3, F2, 0, 0, brickcolor, "Effect", vt())
            prt.Anchored = true
            prt.CFrame = cframe
            local msh = mesh("SpecialMesh", prt, "Sphere", "", vt(0, 0, 0), vt(x1, y1, z1))
...
Â             ElecEffect = function(cff, x, y, z)
...
Â             function FindNearestTorso(Position, Distance, SinglePlayer)
...
Â Â             function CreatePart(Parent, Material, Reflectance, Transparency, BColor, Name, Size)
...
Â     function CreateMesh(Mesh, Part, MeshType, MeshId, OffSet, Scale)
...
Â Â Â     function BlockEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay, Type)
...
Â     function SphereEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
        local prt = CreatePart(workspace, "Neon", 0, 0, brickcolor, "Effect", Vector3.new())
        prt.Anchored = true
        prt.CFrame = cframe
...
Â     function RingEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay,material)
...
Â     function CylinderEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
...
Â     function WaveEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
        local prt = CreatePart(workspace, "Neon", 0, 0, brickcolor, "Effect", Vector3.new())
        prt.Anchored = true
        prt.CFrame = cframe
...
Â     function SpecialEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
        local prt = CreatePart(workspace, "Neon", 0, 0, brickcolor, "Effect", Vector3.new())
        prt.Anchored = true
        prt.CFrame = cframe
...
Â Â     function MoonEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
        local prt = CreatePart(workspace, "Neon", 0, 0, brickcolor, "Effect", Vector3.new())
        prt.Anchored = true
        prt.CFrame = cframe
...
Â     function HeadEffect(brickcolor, cframe, x1, y1, z1, x3, y3, z3, delay)
        local prt = CreatePart(workspace, "Neon", 0, 0, brickcolor, "Effect", Vector3.new())
        prt.Anchored = true
        prt.CFrame = cframe
...
Â     function BreakEffect(brickcolor, cframe, x1, y1, z1)
...
Â             local lerp = function(a, b, t)
        return a * (1 - t) + b * t
    end
Â             function clerp(a,b,t)
...
Â             function QuaternionFromCFrame(cf)
...
Â Â             function QuaternionToCFrame(px, py, pz, x, y, z, w)
...
Â             function weld5(part0, part1, c0, c1)
...
Â             --Example: Torso.Weld.C0 = clerp(Torso.Weld.C0, CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)), 0.4)
Â             function rayCast(Pos, Dir, Max, Ignore)  -- Origin Position , Direction, MaxDistance , IgnoreDescendants
            return game:service("Workspace"):FindPartOnRay(Ray.new(Pos, Dir.unit * (Max or 999.999)), Ignore)
        end
Â Â Â Â Â Â Â     local f = 0
...
Â Â Â                             mouse.KeyDown:connect(function(key)
        if string.byte(key) == 48 then
            b.Size = 40
            Swing = 2
            freefall = 0
Â     coroutine.resume(coroutine.create(function()
        for i = 0,1,0.1 do
            swait()
...
Â     mouse.KeyUp:connect(function(key)
        if string.byte(key) == 48 then
            Swing = 1
            Humanoid.WalkSpeed = 64
            boost = false
...
Â Â Â Â                             mouse.KeyDown:connect(function(key)
...
Â Â     local lastwall = nil
    local jumped = false
Â Â Â Â Â Â             local vwall = false
Â                     mouse.KeyDown:connect(function(key)
...
Â             attack = true
    ...
    RightLeg.Weld.C0 = clerp(RightLeg.Weld.C0, CFrame.new(0.5, -1-0.28*math.cos(sine/4),0+0.32*math.cos(sine/4)) * CFrame.Angles(math.rad(0-104*math.sin(sine/4)), math.rad(0)+ RootPart.RotVelocity.Y / 42, math.rad(0)- RootPart.RotVelocity.Y / 54), 0.3+ Humanoid.WalkSpeed/272)
            ...
            velo:Destroy()
            wait(0.07)
Â     if vwall == false then
Â     RootPart.Velocity = -RootPart.CFrame.lookVector*68 + Vector3.new(0,86,0)
Â     --[[]
            for i = 0,5,0.2 do
    rs:wait()
...
Â Â Â Â         mouse.KeyUp:connect(function(key)
                if key == 'b' and vwall == true then
                vwall = false
                end
                end)
Â Â Â Â Â     function Ldash()
Â Â Â     evadecooldown = true
    attack = true
    k = math.random(1,2) 
    if k == 1 then
...
Â Â Â Â Â Â Â Â Â Â     local NV = Vector3.new(0,0,0)
...
Â Â     end))
Â Â Â     end
Â Â Â Â     function stomp()
        attack = true
        stompsound:Play()
Â     while hitfloor == nil do 
        swait()
        b.Size = 12
...
Â     if spd < 14 then
    Humanoid.Jump = true
Â     if osix == false then
    so("rbxassetid://537371462",RootPart,2,1)
    end
Â Â     RootPart.Velocity = Vector3.new(0,102,0)
    attack = true
    wait(0.08)
    for i = 0,7,0.1 do
        swait()
...
Â     Torso.Weld.C0 = clerp(Torso.Weld.C0, CFrame.new(0, -3, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)), 1)
    for i = 0,5,0.1 do
        swait()
...
Â Â     mouse.KeyDown:connect(function(key)
...
Â Â     local walljump = false
Â Â     function walljumpp()
            local ray = Ray.new(
...
Â         if homed ~= nil and homed:FindFirstChildOfClass("Humanoid").Health > 1 and walljump == false then
...
Â             while homed ~= nil and (homed.Torso.Position-RootPart.Position).magnitude > 8 do
            swait()
            SBall.CFrame = CFrame.new(RootPart.CFrame.p)
...
Â     if string.byte(key) == 32 and hitfloor == nil and attack == true and walljump == true then
    walljump = false
    end
        end)
Â Â Â     mouse.KeyDown:connect(function(key)
...
Â         local TiltVelocity = CFrame.new(RootPart.CFrame:vectorToObjectSpace(RootPart.Velocity))
Â     local rlegray = Ray.new(RightLeg.Position+Vector3.new(0,0.54,0),Vector3.new(0, -1.75, 0))
    local rlegpart, rlegendPoint = workspace:FindPartOnRay(rlegray, Character)
Â     local llegray = Ray.new(LeftLeg.Position+Vector3.new(0,0.54,0),Vector3.new(0, -1.75, 0))
    local llegpart, llegendPoint = workspace:FindPartOnRay(llegray, Character)
Â             local waterthing = Ray.new(RootPart.CFrame.p,Vector3.new(0,-1,0))
        local start, position = workspace:FindPartOnRay(waterthing, character)
Â         if start ~= nil and start.Material == "Water" then
Â         RootPart.Velocity = RootPart.Velocity + Vector3.new(0,6,0)
Â         end
Â     headsunsound.Pitch = 0.76 + Humanoid.WalkSpeed/124
...
Â Â Â Â Â Â Â             if RootPart.Velocity.y > 1 and hitfloor==nil then
    Anim="Jump"
Â Â Â Â Â     if attack==false then
    change = 1
    look = 0
                    Humanoid.CameraOffset = Humanoid.CameraOffset:lerp(Vector3.new(0,0,0),0.15)
...
Â     elseif RootPart.Velocity.y < -1 and freefall <150 and hitfloor==nil then
    Anim="Fall"
    change = 1
    freefall = freefall +0.77
Â Â     if attack==false then
                    Humanoid.CameraOffset = Humanoid.CameraOffset:lerp(Vector3.new(0,0,0),0.15)
...
Â Â Â     elseif RootPart.Velocity.y < -1 and freefall > 150 and hitfloor==nil then
    Anim="FreeFall"
    change = 1
Â Â     if attack==false then
                    Humanoid.CameraOffset = Humanoid.CameraOffset:lerp(Vector3.new(0,0,0),0.15)
...
Â     elseif torvel<1 and hitfloor~=nil then
    Anim="Idle"
    change = 1
    if attack==false and equip == false then
Â                     Humanoid.CameraOffset = Humanoid.CameraOffset:lerp(Vector3.new(0,0,0),0.15)
...
Â Â Â     elseif torvel>1.5 and torvel<70 and hitfloor~=nil then
    Anim="Walk"
...
Â Â             elseif torvel>=70 and torvel<200 and hitfloor~=nil then
            Anim="Run"
...
Â Â Â                     elseif torvel>=200 and hitfloor~=nil then
            Anim="MachRun"
...
Â Â Â     end
Â Â Â Â Â Â Â Â Â     if 0 < #Effects then
        for e = 1, #Effects do
...
Â for i, Part in ipairs(FakeCharacter:GetDescendants()) do
	if Part:IsA("BasePart")then
		Part.Transparency = fake_transparency
	end
end
Â for i, Decal in ipairs(FakeCharacter:GetDescendants()) do
	if Decal:IsA("Decal")then
		Decal.Transparency = fake_transparency
	end
end
Â Player.Character = FakeCharacter
Â Â local function MotorAngle()
...
Â RunService.Heartbeat:Connect(function()
	SetAngles()
	MotorAngle()
	RealChar.HumanoidRootPart.CFrame = FakeCharacter.Torso.CFrame
end)
Â RunService.PreSimulation:Connect(function()
	BaseCol()
end)
Â LoadAllAnimations()