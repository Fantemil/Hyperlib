--// Trident Survival v1.2b [Private]
...
repeat task.wait() until game:IsLoaded()
  ...
  for I,V in pairs(getgc(true)) do
    if type(V) == "function" then
      if debug.getinfo(V).name == "CreateAlert" then
        V("velocity.cc (private) loaded! - [trident hacked]", Color3.fromRGB(255,0,0))
      end
    end
  end
  ...
  --
  local Build = "private";local Color = "#FF0000";local Ver = "v1.2b"
  if Build == "private" then Color = '#FF0000' Ver = "v1.2b" end
...
function Functions:Predict()
  ...
  if Functions:GetClosest() ~= nil then
    ...
    if Velocity and TimeOfFlight then
      ...
      Prediction = (Velocity * (TimeOfFlight*8)) * .65
    end
...
function InventoryViewer:CreateText(IsTitle,String,SlightIndex)
  DrawingText = Functions:Draw("Text",{Visible=false,Position=Vector2.new(InventoryViewer.Background.Position.X+14,(InventoryViewer.Background.Position.Y+28)+((#InventoryViewer.Texts - 1)*18)),Outline=true,Center=false,Font=2,Size=15,Text=String,Color=Color3.fromRGB(255,255,255)})
...
SilentTab:AddToggle('SilentAim',{Text='enabled',Default=true}):AddKeyPicker('SilentKey', {Default='MB1',SyncToggleState=true,Mode='Hold',Text='Silent Aim',NoUI=false}):OnChanged(function(Value)
...
PlayerSettingsTab:AddToggle('',{Text='box filled',Default=true}):AddColorPicker('BoxesFilledColor',{Default=Color3.fromRGB(0, 0, 0),Title='Color'}):OnChanged(function(Value)
Esp.Settings.BoxesFilled = Value
end)
Options.BoxesFilledColor:OnChanged(function(BoxesFilledColor)
Esp.Settings.BoxesFilledColor = BoxesFilledColor
end)
...
CustomSkyTab:AddToggle('Leaves', {Text = 'remove leaves', Default = false, Tooltip = "Leaves On/Off"})
local function UpdateLeavesTransparency()
if LightingEnabled and Toggles.Leaves.Value == true then
