if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Solara Yield" is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Solara is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end

currentVersion = "Executor"
...
Title.TextSize = 18
Title.Text = "Solara " .. currentVersion
...
Logo.Image = "rbxassetid://17608633242"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/realsolara")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/realsolara)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Solara Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Solara support server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		...
		notify('Discord Invite', 'https://discord.gg/realsolara')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Solara Yield may have issues or features that are not functioning correctly.") end
end)